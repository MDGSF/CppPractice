...
static const uint32_t uxHeapStructSize =
    ((sizeof(SBlock) + (portBYTE_ALIGNMENT - 1)) & ~portBYTE_ALIGNMENT_MASK);
...
SHeapRegion xHeapRegions[] = {
    {ucHeap, sizeof(ucHeap)}, {NULL, 0}  // Terminates the array.
};
...
void vPortDefineHeapRegions(const SHeapRegion* const pxHeapRegions) {
  ...
  while (pxHeapRegion->xSizeInBytes > 0) {
    ...
    pxFirstFreeBlockInRegion->xBlockSize =
        uiAddress - (uint32_t)pxFirstFreeBlockInRegion;
